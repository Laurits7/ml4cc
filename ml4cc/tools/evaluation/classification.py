...
def thresholded_roc_curve(truth, predictions, n_classifier_cuts=100, signal: str = "both") -> dict:
    print("Calculating ROC curve...")
    thresholds = np.linspace(start=0, stop=1, num=n_classifier_cuts + 1)
    tpr = []
    fpr = []
    if signal == "both":
        truth = ak.values_astype((truth == 1) + (truth == 2), int)
    elif signal == "primary":
        truth = ak.values_astype((truth == 1), int)
    else:
        raise ValueError("Signal type must be 'both' or 'primary'")
