...
class MultiComparisonPlot:
    ...
    def _add_histograms(self, ax, true: np.array, pred: float, energy: str, print_legend: bool = False):
        hep.histplot(
            to_bh(true, bins=self.bins),
            ax=ax, density=True, histtype='fill', alpha=0.3, hatch="//", label='Target')
        hep.histplot(
            to_bh(pred, bins=self.bins),
            ax=ax, density=True, histtype='fill', alpha=0.3, hatch="\\\\", label='Reconstructed')
        ax.set_title(f"{energy} GeV")
        if print_legend:
